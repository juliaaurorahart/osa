...
To enable private cloud saves for `osa.juliaaurorahart.com`, create a Cloudflare Zero Trust Access application for that hostname (allow only the owner's email with One-time PIN), copy its team domain and audience into `functions/access-config.ts`, then bind a D1 database as `OSA_DB` in the Pages project and apply `migrations/0001_boards.sql`. Until Access is configured, the API fails closed and OSA stays browser-only.
