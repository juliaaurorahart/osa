...
try {
  ...
  assert.match(file.url, /^\/api\/assets\?/)
  assert.equal(new URL(file.url, 'https://osa.test').searchParams.get('boardId'), one.board.id)
  const readFile = (url, email) => getAsset(apiContext(new Request(new URL(url, 'https://osa.test')), email))
  ...
  const forgedUrl = new URL(file.url, 'https://osa.test')
