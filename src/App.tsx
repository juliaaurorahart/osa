...
function OsaObjectNode({ id, data, dragging }: NodeProps<OsaNode>) {
  return (
    <div className={`osa-object${dragging ? ' dragging' : ''}`}>
      ...
      {data.showGaiaRoot && <span className="gaia-root" aria-label="Rooted in Gaia"><i className="root-shoot shoot-left" /><i className="root-shoot shoot-right" /><i className="root-shoot shoot-tip" /></span>}
...
function Playground() {
  ...
  const [fieldInkWidth, setFieldInkWidth] = useState(3)
  ...
  const eraseFieldInkAt = (point: Point) => {
    const reach = 20 / fieldZoom
    setFieldStrokes((current) => current.filter((stroke) => !stroke.points.some((ink) => Math.hypot(ink.x - point.x, ink.y - point.y) < reach)))
  }
  ...
  const finishFieldSketch = (event?: React.PointerEvent<HTMLDivElement>) => {
    ...
    setFieldStrokes((current) => [...current, { id: crypto.randomUUID(), points, color: fieldInkColor, width: fieldInkWidth }])
